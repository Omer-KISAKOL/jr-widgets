...
import TermsAndConditions from '../../styles/TermsAndConditions.css'
...
class ConfigurationForm extends React.Component {
  ...
  componentDidMount() {
    ...
    // let AIButton = document.querySelector("#app_wizards > div > div:nth-child(2) > div.mainCard.noSpace.mb-18 > form > div > div.tabContent > div:nth-child(3) > div > div.column.five.seven > button")
    let AIYes = document.querySelector("#\\38 69d4f9c07993e2528ef3ece\\.createWithAI\\.0");
    let AINo = document.querySelector("#\\38 69d4f9c07993e2528ef3ece\\.createWithAI\\.1");
    let sideSelectedIndustry = document.querySelector("#app_wizards > div > div:nth-child(2) > div.mainCard.noSpace.mb-18 > form > div > div.tabContent > div:nth-child(2)");
    let sideAIPrompt = document.querySelector("#app_wizards > div > div:nth-child(2) > div.mainCard.noSpace.mb-18 > form > div > div.tabContent > div:nth-child(4)");
    let other = document.querySelector("#app_wizards > div > div:nth-child(2) > div.mainCard.noSpace.mb-18 > form > div > div.tabContent > div:nth-child(3)");
    let selectedIndustryValue = document.getElementById('869d4f9c07993e2528ef3ece.selectIndustry');

    if (AINo.checked) {
      sideSelectedIndustry.style.height = "0";
      sideSelectedIndustry.style.opacity = "0";
      sideSelectedIndustry.style.visibility = "hidden";
      sideSelectedIndustry.style.overflow = "hidden";
      sideSelectedIndustry.style.padding = "0";

      sideAIPrompt.style.height = "0";
      sideAIPrompt.style.opacity = "0";
      sideAIPrompt.style.visibility = "hidden";
      sideAIPrompt.style.overflow = "hidden";
      sideAIPrompt.style.padding = "0";

      other.style.height = "0";
      other.style.opacity = "0";
      other.style.visibility = "hidden";
      other.style.overflow = "hidden";
      other.style.padding = "0";

      selectedIndustryValue.value = "Choose";

    }

    if (AIYes.checked) {
      sideSelectedIndustry.style.height = "auto";
      sideSelectedIndustry.style.opacity = "1";
      sideSelectedIndustry.style.visibility = "visible";
      sideSelectedIndustry.style.overflow = "hidden";
      sideSelectedIndustry.style.padding = "20px";

      sideAIPrompt.style.height = "auto";
      sideAIPrompt.style.opacity = "1";
      sideAIPrompt.style.visibility = "visible";
      sideAIPrompt.style.overflow = "hidden";
      sideAIPrompt.style.padding = "20px";

      other.style.height = "auto";
      other.style.opacity = "1";
      other.style.visibility = "visible";
      other.style.overflow = "hidden";
      other.style.padding = "20px";
    }

    AINo.addEventListener('change', (e) => {
      if (e.target.checked) {
        sideAIPrompt.style.transition = "height 0.5s, opacity 0.5s, padding 0.5s";
        sideSelectedIndustry.style.transition = "height 0.5s, opacity 0.5s, padding 0.5s";
        other.style.transition = "height 0.5s, opacity 0.5s, padding 0.5s";
        ...
        if (AINo.checked && selectedIndustryValue.value !== "Other") {
          other.style.height = "0";
          other.style.opacity = "0";
          other.style.visibility = "hidden";
          other.style.overflow = "hidden";
    ...
    AIYes.addEventListener('change', (e) => {
      if (e.target.checked) {
        sideAIPrompt.style.transition = "height 0.5s, opacity 0.5s, padding 0.5s";
        sideSelectedIndustry.style.transition = "height 0.5s, opacity 0.5s, padding 0.5s";
        other.style.transition = "height 0.5s, opacity 0.5s, padding 0.5s";

        if (AIYes.checked && selectedIndustryValue.value !== "Other") {
          other.style.height = "0";
          other.style.opacity = "0";
          other.style.visibility = "hidden";
          other.style.overflow = "hidden";
        ...
        if (selectedIndustryValue.value === "Other") {
          other.style.visibility = "visible";
          other.style.height = "auto";
          other.style.opacity = "1";
          other.style.padding = "20px";
        ...
        selectedIndustryValue.addEventListener('change', (e) => {
    ...
    // let areaText = document.querySelector("#\\38 69d4f9c07993e2528ef3ece\\.termsContent");

    // if (this.props.onareaTextClick) {
    //     this.props.onareaTextClick(areaText);
    // }
    ...
    //           bubbles: true
    //       });
    //       areaText.dispatchEvent(keypressEvent);
  ...
  handleChange(e) {
    // console.log(this.state.values, 'STATE');
    // console.log(e.target.value, 'NEW STATE')
  ...
  createQuestionElement(element) {
    ...
    const AIButton = (name, defaultText, AIvalue) => {
      ...
      if (element.attributes.id === `${AI_TermsandCondition_ID}.AIPrompt`) {
        var AIvalue = formControls[formControls.length - 1].props.value;
      }
      ...
      const sendPrompt = async () => {
        ...
        let otherIndustry = document.querySelector("#\\38 69d4f9c07993e2528ef3ece\\.otherIndustry").value;
        let selectedIndustry = document.getElementById('869d4f9c07993e2528ef3ece.selectIndustry').value;
        if (selectedIndustry === "Choose") {
          selectedIndustry = '';
        } else if (selectedIndustry === "Other") {
          selectedIndustry = otherIndustry;
        }else{
          selectedIndustry = document.getElementById('869d4f9c07993e2528ef3ece.selectIndustry').value;
        }


        formData.append('prompt', AIvalue);
        formData.append('sector', selectedIndustry);
        formData.append('formID', formId);

        try {
          const response = await fetch('https://rc-akdemir.jotform.dev/intern-api/API/widgets/terms_and_conditions/form/ask_ai', {
            method: 'POST',
            body: formData,
          });
      ...
      const onClick = async () => {
        ...
        let AIPromptArea = document.querySelector("#\\38 69d4f9c07993e2528ef3ece\\.AIPrompt");
        AIPromptArea.addEventListener("input", () => {
          AIPromptArea.style.border = '1px solid #292D3C';
        })
        if (AIPromptArea.value.trim().length !== 0) {
          try {
            ...
            this.setState(oldState => {
              ...
              const index = newState.values.findIndex(val => val.name === 'termsContent');
              newState.values[index].value = response.content;
              if (this.props.onareaTextClick) {

                this.props.onareaTextClick(newState.values);
          ...
          AIPromptArea.style.border = '2px solid #e46962';
        }
      };
      ...
      const AIButton = (
          <>
            <Button
                className="mt-6 mb-4 w-full h-10 flex items-center justify-center"
                {...buttonProps}
                onClick={onClick}
            >
              <div className='flex items-center justify-center gap-1 '>
                <IconAiFilled className="h-5 w-5 " />
                Generate with AI
              </div>
            </Button>
          </>
    ...
    if (element.attributes.id === `${AI_TermsandCondition_ID}.AIPrompt`) {
      formControls.push(AIButton('AIButton'));
    }
