...
class ConfigurationForm extends React.Component {
  ...
  createQuestionElement(element) {
    ...
    const AIButton = (name, defaultText, AIvalue) => {
      ...
      if (element.attributes.id === `${AI_TermsandCondition_ID}.termsContent`) {
        
        var AIcontent = formControls[formControls.length-1].props.value;
      ...
      formData.append(AIvalue, getValue('AIPrompt'));
      formData.append(AIcontent, getValue('termsContent'));

      const sendPrompt = async () => {
        formData.delete('prompt');
        formData.append('prompt', AIvalue);
        formData.append('prompt', AIcontent);
        try {
      ...
      const onClick = async () => {
        ...
        try {
          ...
          console.log('Test Result:', this.state.buttonStates[name].AIcontent);
    ...
    if (element.attributes.id === `${AI_TermsandCondition_ID}.AIPrompt`) {
      // console.log(formControls[formControls.length-1], 'controls-1');
      console.log(formControls[formControls.length-1].props.value, '---value');

      formControls.push(AIButton('AIButton'));
      // formControls.push(hr);
      // formControls.push(data);
    }
    if (element.attributes.id === `${AI_TermsandCondition_ID}.termsContent`) {
      // console.log(formControls[formControls.length-1], 'controls-1');
      console.log(formControls[formControls.length-1].props.value, '---value////');
